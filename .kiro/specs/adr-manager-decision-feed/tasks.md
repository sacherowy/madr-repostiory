...
- INFRA (after 5.4): The web unit suite crossed the sandbox's 4096 fd cap because every real-backend test makes git commits and the global gitconfig forces `commit.gpgsign true` via `/tmp/code-sign`; the signing subprocesses exhaust fds under parallel load ("too many open files" in fixture setup). Fixed test-only in `apps/web/src/test/setup.ts` by setting `GIT_CONFIG_COUNT/KEY_0/VALUE_0` to disable `commit.gpgsign` for git subprocesses in the test worker (does NOT touch app commit behavior). Contributing factor: the container opens 3 unclosed better-sqlite3 connections per build (searchIndex/embeddingStore/summaryStore — the 3rd added by this feature). If the e2e suite (9.x) hits the same wall, apply the same env override in its harness. Full web suite: 423/423 green after the fix.
