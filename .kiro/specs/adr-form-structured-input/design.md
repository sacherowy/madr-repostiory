...
- `apps/e2e/tests/migrated-fixture-content.spec.ts` — **discovered during task 5, not part of the original plan**: its editor-side assertion loop (`SECTION_TESTID_KEYS.map(key => \`${key}-textarea\`)`) also asserted on `considered-options-textarea`/`pros-and-cons-of-the-options-textarea`, which no longer exist as plain textareas. Its second loop (asserting `${key}-block` in the read-only History viewer) is unaffected — `HistoryTimeline` is a separate, untouched component that still renders all 8 canonical section blocks. Fix: the editor-side loop must skip `considered-options`/`pros-and-cons-of-the-options`; the history-side loop is unchanged.
