...
- [x] 13.1 (P) Use the nine discrete fields instead of the single body field when creating, saving, and indexing an ADR
...
- [x] 15.2 (P) Build similarity embedding text from the combined section content instead of the single body field
...
- [x] 16.1 (P) Replace the single body textarea with one field per MADR section plus one for unmapped content in the edit form
...
- [x] 17.1 (P) Replace the single body paragraph with one read-only block per MADR section plus one for unmapped content in the history viewer
...
- [x] 18.1 Add end-to-end coverage for creating and editing an ADR through its nine discrete fields
...
- [x] 18.2 Verify the migrated example fixture's content displays consistently as catch-all content across the app
...
- Task 18.1 retrofitted `apps/e2e/tests/adr-lifecycle.spec.ts` from the removed single `body-textarea` model to the nine-discrete-field model: confirms all 8 section fields render empty on create with the two required sections' labels distinguishable from the six optional ones; fills in and saves 3 different sections independently plus the catch-all field, then confirms each round-trips through a fresh reload while untouched sections stay empty; updates the raw-API conflict-forcing PUT payload from the old `body:` field to the new `AdrSections`-shaped `UpdateAdrRequest` (8 sections + `additionalContent`) via a `sectionsPayload()` helper; preserves all pre-existing decisionMakers/consulted/informed round-trip, rejected-status-without-relation, and save-conflict detect/reload-latest/recover coverage unchanged. RED phase confirmed (`body-textarea` no longer in the DOM, original spec timed out); GREEN confirmed (rewritten spec passes, ~17-22s) by implementer, reviewer, and parent verification independently, all matching. The full e2e suite surfaced one pre-existing, out-of-boundary failure: `apps/e2e/tests/search.spec.ts` (line 35) still references the removed `body-textarea` testid and fails the same way the original `adr-lifecycle.spec.ts` did pre-retrofit. This file is not owned by 18.1 or 18.2 per tasks.md's task descriptions; the reviewer independently proved via `git stash`/`git stash pop` that this failure is pre-existing (reproduces identically against baseline, unrelated to this task's diff). This is a known gap a future task or follow-up spec should pick up — flagging here rather than silently leaving it undocumented. No FD-exhaustion signing/network issue recurred during this task (e2e tests use the app's own local `simple-git` instance via the API server, not the host's commit-signing/push path).
- Task 13.1's correct `AdrEditingService` retrofit (validating the 2 MADR-required sections instead of `body`) broke 26 tests across 6 `apps/api/src/routes/*.test.ts` files that still built PUT payloads with only `body:`. Root-caused as a pure stale-test-fixture problem — the route layer (`adrs.ts`) is an untyped pass-through with no body-specific logic — so the fix was test-fixture-only: `adrs.test.ts`, `relations.test.ts`, `search.test.ts`, `similarity.test.ts`, `history.test.ts`, `compare.test.ts` payloads/assertions updated from `body:` to `contextAndProblemStatement`/`decisionOutcome`. `adrs.test.ts` additionally needed all 6 optional sections + `additionalContent` set to `""` in its PUT payloads (the only file doing deep-equality assertions) to work around the pre-existing `"undefined"`-string data-corruption bug noted above, which 13.1 does not own fixing in production code. After this fix, `apps/api` is green except 1 expected, pre-existing failure in `compare.test.ts` (`byField("body").differs`), owned by task 14.1 (`ComparisonService.FIELD_NAMES` still includes `"body"`) — not a new regression.
- Task 15.2 closed out the `adr.body`-removal ripple-effect chain documented above (12.2 → 13.1 → 14.1 → 15.1 → 15.2). `similarityService.ts`'s `vectorFor()` now builds embedding text via the same `combinedSectionText(adr, adr.additionalContent)` helper `reindex.ts` consumes (re-exported from `packages/core/src/index.ts` since 15.1), so the two call sites cannot drift apart. As of this task, `pnpm --filter @adr/core test` (118/118), `pnpm --filter @adr/core build` (0 errors), and `pnpm --filter @adr/api test` (107/107) are all fully green with zero remaining known/pre-existing `adr.body` failures in either package. Two `adr.body` references remain outside this chain's scope, in `apps/web` (`AdrEditor.tsx`, `HistoryTimeline.tsx`), expected to be owned by tasks 16.1 and 17.1 respectively.
- Task 16.1 replaced `EditAdrForm`'s single `body` textarea with a `MADR_SECTIONS.map(...)`-driven set of 8 labeled section textareas (canonical order, metadata-derived required/optional indicator) plus an `additionalContent` textarea, fixing the last `adr.body` reference inside `AdrEditor.tsx`/`AdrEditor.test.tsx` flagged by 15.2. Sandbox note unrelated to the diff: during this task's verification, `pnpm --filter @adr/web test` (and any other command spawning real `simple-git` commits) began failing with `fatal: failed to write commit object` / `too many open files`, traced to the long-lived `environment-manager` host process (PID 537) holding ~4094/4096 of its own file-descriptor ulimit, inherited by child processes; confirmed environmental (not a regression) by stashing the diff and reproducing the identical failure against unmodified baseline code. `pnpm --filter @adr/web build` (vite) and `npx tsc --noEmit -p apps/web/tsconfig.json` (zero errors in either changed file) both ran clean and don't spawn git operations. The task's reviewer captured a clean, independently-fresh `AdrEditor.test.tsx` 17/17 pass plus stash/pop RED-phase verification earlier in their own session, before the leak fully saturated — that run, combined with this task's build/typecheck confirmation and baseline-equivalence proof, is the evidence basis for completion. Any future task in this spec needing to run `apps/web`'s git-backed tests should expect this same environmental failure mode and re-verify via the same stash-comparison technique rather than assuming a regression. Separately, the same FD exhaustion also blocked the actual `git commit`/`git push` for this task's changes (signing helper and proxy DNS lookups both failed identically); the user explicitly authorized a one-time `--no-gpg-sign` bypass to land the commit, after which the FD pressure on the network path cleared on its own after several minutes of retrying, allowing a normal signed push flow to resume for subsequent tasks.
- Task 17.1 fully closed out the `adr.body`-removal ripple-effect chain in `apps/web`: `HistoryTimeline.tsx` now maps `MADR_SECTIONS` into 8 labeled read-only blocks (canonical order, required/optional indicator from shared metadata) plus a conditional `additionalContent` block (rendered only when non-empty, since this is a read-only viewer rather than an always-present editable field like 16.1), replacing the last `<p data-testid="history-version-body">{selected.adr.body}</p>` reference in the codebase. No `adr.body`-shaped references remain anywhere in `@adr/core`, `apps/api`, or `apps/web`. This task did not encounter the FD-exhaustion signing/network issue documented under 16.1 — `pnpm --filter @adr/web test -- HistoryTimeline.test.tsx` (6/6), `pnpm --filter @adr/web build`, and `npx tsc --noEmit -p apps/web/tsconfig.json` all ran cleanly on the first attempt for both the reviewer and the parent's independent verification.
- Task 18.2 added `apps/e2e/tests/migrated-fixture-content.spec.ts`, a new sibling to `migrated-fixture-title.spec.ts` (which already covered title display, not content). It reads `examples/0001-uzycie-gita-jako-zrodla-prawdy.md` verbatim (sliced after frontmatter) and seeds it directly into the e2e temp repo via `simple-git`, bypassing the API (whose `serializeAdr` can only emit canonical new-style frontmatter). Confirms the fixture's three Polish headings (`Kontekst`/`Decyzja`/`Konsekwencje`, none matching a canonical MADR heading) land entirely in `additionalContent` per `splitSections`'s catch-all contract: all 8 section fields are empty and `additional-content-textarea` contains the original Polish text with no duplication, in both the editor and the history viewer (`panel-tab-history` → select the seeded commit by `data-sha` → `history-version-content`), plus an explicit normalized cross-surface equality check between the editor's and history viewer's `additionalContent` text. This is the proof case Requirement 5.5 and design.md call out: a legacy/non-conforming body is read losslessly via the catch-all rather than dropped or mis-split. Parent verification reran the test fresh (1/1 pass, ~15-20s) and `npx tsc --noEmit -p apps/e2e/tsconfig.json` (0 errors) independently of the implementer's and reviewer's runs, matching their results; `git status`/`git diff --stat HEAD` confirmed the diff is scoped to exactly the one new untracked file with no tracked-file changes. No FD-exhaustion signing/network issue recurred during this task either.
- Task 16.1 replaced `EditAdrForm`'s single `body` textarea with a `MADR_SECTIONS.map(...)`-driven set of 8 labeled section textareas (canonical order, metadata-derived required/optional indicator) plus an `additionalContent` textarea, fixing the last `adr.body` reference inside `AdrEditor.tsx`/`AdrEditor.test.tsx` flagged by 15.2. Sandbox note unrelated to the diff: during this task's verification, `pnpm --filter @adr/web test` (and any other command spawning real `simple-git` commits) began failing with `fatal: failed to write commit object` / `too many open files`, traced to the long-lived `environment-manager` host process (PID 537) holding ~4094/4096 of its own file-descriptor ulimit, inherited by child processes; confirmed environmental (not a regression) by stashing the diff and reproducing the identical failure against unmodified baseline code. `pnpm --filter @adr/web build` (vite) and `npx tsc --noEmit -p apps/web/tsconfig.json` (zero errors in either changed file) both ran clean and don't spawn git operations. The task's reviewer captured a clean, independently-fresh `AdrEditor.test.tsx` 17/17 pass plus stash/pop RED-phase verification earlier in their own session, before the leak fully saturated — that run, combined with this task's build/typecheck confirmation and baseline-equivalence proof, is the evidence basis for completion. Any future task in this spec needing to run `apps/web`'s git-backed tests should expect this same environmental failure mode and re-verify via the same stash-comparison technique rather than assuming a regression.
- Final `/kiro-validate-impl` gate (after all 18 sub-tasks reached `[x]`) ran a full fresh `pnpm --filter @adr/web test` and surfaced a real, previously-undetected regression from task 11.2's `body`→`AdrSections` type change: 8 leftover `apps/web` test files (`App.test.tsx`, `client.test.ts`, `AdrCompareView.test.tsx`, `CompareLauncher.test.tsx`, `VersionDiffView.test.tsx`, `RelationsPanel.test.tsx`, `SearchPanel.test.tsx`, `SimilarityPanel.test.tsx`) still built `UpdateAdrRequest` payloads with the removed `body:` field, producing 35 real test failures (`editingService.ts`'s required-section validation rejecting payloads missing `contextAndProblemStatement`/`decisionOutcome`) plus matching `tsc` errors invisible to `pnpm -r build`/`pnpm -r test` for the same reason documented under task 9.1 (vitest's esbuild transform skips type-checking; `apps/web`'s build script is `vite build` only). None of tasks 11-18 owned these specific files' boundary, so this was fixed as final-gate remediation rather than retroactively reopening a numbered task: replaced `body:` with `contextAndProblemStatement` (carrying the original free-text content), `decisionOutcome: "Proceed."`, the other 6 sections + `additionalContent` set to `""`. Two latent secondary bugs surfaced and fixed in the same pass: `App.test.tsx` referenced a stale `body-textarea` test-id (real ids are `${kebab-section}-textarea`, fixed by task 16.1) and `AdrCompareView.test.tsx` asserted on a stale 8-field comparison shape (real list is 16 fields per `ComparisonService.FIELD_NAMES` after task 14.1's retrofit). Diff-viewer and search/similarity tests were checked individually to confirm distinct per-version content was preserved (not collapsed to identical placeholders, which would have made `differs:true`/keyword-match assertions pass vacuously). Reviewer and parent independently reran the full suite fresh: 234/234 passing (was 35 failed/199 passed before), and `npx tsc --noEmit -p apps/web/tsconfig.json` clean except the pre-existing, unrelated `main.tsx` CSS-import `TS2882` errors (confirmed pre-existing via `git stash` baseline check — `main.tsx` not touched by this diff, last modified by an unrelated commit from a different spec). Diff scoped to exactly these 8 test files, zero production code.
- The same final gate's full `pnpm e2e` run surfaced one more instance of the identical regression class, previously flagged in task 18.1's note as a known, then-undiagnosed-as-local gap: `apps/e2e/tests/search.spec.ts` (line 35) filled the removed `body-textarea` testid as part of its own fixture-creation step, timing out. On reflection this is locally owned by this spec (task 16.1's UI retrofit caused it), not a permanent out-of-boundary gap, so it was fixed directly rather than left documented-but-broken: replaced the single `body-textarea` fill with `context-and-problem-statement-textarea` (carrying the original token text) plus a `decision-outcome-textarea` fill (`"Proceed."`), since `AdrEditor`'s save submits all fields and the server rejects a save missing either required section. Verified via a fresh isolated `search.spec.ts` run (1/1 pass) and `npx tsc --noEmit -p apps/e2e/tsconfig.json` (0 errors), then the full `pnpm e2e` suite re-run clean end to end with no remaining failures.
